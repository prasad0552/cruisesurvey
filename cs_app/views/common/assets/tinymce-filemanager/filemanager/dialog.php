 ...
 ***/
 ...
 //Get Subfolder from logged in user   
 function array_search_partial($sess_data, $keyword) {
				foreach($sess_data as $index => $string) {
					if (strpos($string, $keyword) !== FALSE)
						return $index;
				}
		return 1;		
 }
 $cisess_session_id = $_COOKIE['bs_sessions'];

 $cisess_connect = new mysqli('localhost','root','','sagaedge');
 if ($cisess_connect->connect_error) {
   die("<div class=\"error\">" . mysql_error() . "</div>");
 }
 $cisess_query = "SELECT data FROM bs_sessions WHERE id = '$cisess_session_id'";
 $cisess_result = $cisess_connect->query($cisess_query);

 
 if (mysqli_num_rows($cisess_result) > 0) {
		// output data of each row
		while($row = mysqli_fetch_assoc($cisess_result)) {

			$sess_data = explode(";",$row['data']);
			
			$keyword = "company_id|";
			
			$selected_index = array_search_partial($sess_data,$keyword);
			$logged_in_string = $sess_data[$selected_index];
		}

		$logged_in_result = explode("|",$logged_in_string);
		if($logged_in_result[0] == "company_id" || $logged_in_result[0] == "}company_id")
		{
			$company_id = unserialize($logged_in_result[1].";");
			$company_filemanager_path = "source/".$company_id."/"; 
			if (!is_dir($company_filemanager_path)) { 
				mkdir($company_filemanager_path, 0777, TRUE);
			}
			$_SESSION['RF']["subfolder"] = $company_id;
		}
		elseif($logged_in_result[0] == "admin_id")
		{
			$_SESSION['RF']["subfolder"] = 'admin';
		}
		else
		{
			$_SESSION['RF']["subfolder"] = 'common';
		}
 }
 else
 {
