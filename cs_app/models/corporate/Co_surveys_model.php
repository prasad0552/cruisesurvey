...
class Co_surveys_model extends CI_Model {
	...
	function checkQuestionCategoryExists($condition)
	{
	  	if(is_array($condition) && count($condition) > 0 )
		{
			...
			$query=$this->corporate_db->get('question_categories');
	...
	{
	  	$this->corporate_db->insert('question_categories',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:question_categories ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getQuestionCategoryByID($category_id)
	{
			...
			$condition .= " AND ?:question_categories.category_id = '".$category_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:question_categories ".$join." WHERE ".$condition);
	...
	function updateQuestionCategory($update_data,$update_cond)
	{
	  	if(is_array($update_cond) && count($update_cond) >0 )
		{
			...
			$this->corporate_db->update('question_categories',$update_data);
		}
	}
	...
	function deleteQuestionCategory($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('question_categories');
	...
	function checkSurveyExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveyByID($survey_id)
	{
			...
			$condition .= " AND ?:survey.survey_id = '".$survey_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey".$join." WHERE ".$condition);
	...
	function updateSurvey($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey',$update_data);
		}
	}
	...
	function deleteSurvey($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey');
	...
	function checkSurveyDefaultTemplateExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_default_templates');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_default_templates',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_templates ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveyDefaultTemplateByID($template_id)
	{
			...
			$condition .= " AND ?:survey_default_templates.template_id = '".$template_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_templates".$join." WHERE ".$condition);
	...
	function updateSurveyDefaultTemplate($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_default_templates',$update_data);
		}
	}
	...
	function deleteSurveyDefaultTemplate($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_default_templates');
	...
	function checkSurveyDefaultSectionExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_default_sections');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_default_sections',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_sections ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveyDefaultSectionByID($section_id)
	{
			...
			$condition .= " AND ?:survey_default_sections.section_id = '".$section_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_sections".$join." WHERE ".$condition);
	...
	function updateSurveyDefaultSection($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_default_sections',$update_data);
		}
	}
	...
	function deleteSurveyDefaultSection($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_default_sections');
	...
	function checkSurveyDefaultQuestionExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_default_questions');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_default_questions',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_questions ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveyDefaultQuestionByID($question_id)
	{
			...
			$condition .= " AND ?:survey_default_questions.question_id = '".$question_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_questions".$join." WHERE ".$condition);
	...
	function updateSurveyDefaultQuestion($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_default_questions',$update_data);
		}
	}
	...
	function deleteSurveyDefaultQuestion($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_default_questions');
	...
	function checkSurveyDefaultQuestionOptionExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_default_question_options');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_default_question_options',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_question_options ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveyDefaultQuestionOptionByID($option_id)
	{
			...
			$condition .= " AND ?:survey_default_question_options.option_id = '".$option_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_default_question_options".$join." WHERE ".$condition);
	...
	function updateSurveyDefaultQuestionOption($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_default_question_options',$update_data);
		}
	}
	...
	function deleteSurveyDefaultQuestionOption($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_default_question_options');
	...
	function checkSurveySectionExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_sections');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_sections',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_sections ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveySectionByID($section_id)
	{
			...
			$condition .= " AND ?:survey_sections.section_id = '".$section_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_sections".$join." WHERE ".$condition);
	...
	function updateSurveySection($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_sections',$update_data);
		}
	}
	...
	function deleteSurveySection($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_sections');
	...
	function checkSurveyQuestionExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_questions');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_questions',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_questions ".$join." WHERE ".$condition." ".$group_by." ".$order." ".$limit);
	...
	function getSurveyQuestionByID($question_id)
	{
			...
			$condition .= " AND ?:survey_questions.question_id = '".$question_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_questions".$join." WHERE ".$condition);
	...
	function updateSurveyQuestion($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_questions',$update_data);
		}
	}
	...
	function deleteSurveyQuestion($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_questions');
	...
	function checkSurveyQuestionOptionExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('survey_question_options');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('survey_question_options',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_question_options ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function getSurveyQuestionOptionByID($option_id)
	{
			...
			$condition .= " AND ?:survey_question_options.option_id = '".$option_id."'";
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:survey_question_options".$join." WHERE ".$condition);
	...
	function updateSurveyQuestionOption($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('survey_question_options',$update_data);
		}
	}
	...
	function deleteSurveyQuestionOption($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('survey_question_options');
	...
	function checkGuestSurveyReportExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('guest_survey_reports');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('guest_survey_reports',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:guest_survey_reports ".$join." WHERE ".$condition." ".$group_by." ".$order." ".$limit);
	...
	function updateGuestSurveyReport($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('guest_survey_reports',$update_data);
		}
	}
	...
	function deleteGuestSurveyReport($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('guest_survey_reports');
	...
	function checkGuestSurveyAnswerExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('guest_survey_answers');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('guest_survey_answers',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:guest_survey_answers ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function updateGuestSurveyAnswer($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('guest_survey_answers',$update_data);
		}
	}
	...
	function deleteGuestSurveyAnswer($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('guest_survey_answers');
	...
	function checkGuestSurveyCommentExists($condition)
	{
		...
		{
			$this->corporate_db->where($condition);
			$query=$this->corporate_db->get('guest_survey_comments');
			return $query->num_rows();
		}
	}
	...
	{
	  	$this->corporate_db->insert('guest_survey_comments',$insert_data);
		return $this->corporate_db->insert_id();
	}
	...
	{	
			...
			if(!empty($params['per_page']))
			{
	  		...
	  		$result = $this->corporate_db->query("SELECT ".implode(',',$fields)." FROM ?:guest_survey_comments ".$join." WHERE ".$condition." ".$order." ".$limit);
	...
	function updateGuestSurveyComment($update_data,$update_cond)
	{
		...
		{
			$this->corporate_db->where($update_cond);
			$this->corporate_db->update('guest_survey_comments',$update_data);
		}
	}
	...
	function deleteGuestSurveyComment($conditions)
	{
	  		if(count($conditions)>0)		
	 		{
				...
				$this->corporate_db->delete('guest_survey_comments');
